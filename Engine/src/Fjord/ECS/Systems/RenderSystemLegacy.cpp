...
void SpritesBuffer::Init() {
  ...
  // Setup vertex buffer
  auto vBuffer =
      CreateRef<VertexBuffer>(MAX_VERTEX_COUNT * sizeof(SpriteVertexData));
  BufferLayout layout = {
  ...
  auto iBuffer =
      CreateRef<IndexBuffer>(quadIndices, MAX_QUADS_COUNT_FOR_CALL * 6);
...
static bool Compare(
    const std::pair<TransformComponent &, SpriteRendererComponent &> &first,
    const std::pair<TransformComponent &, SpriteRendererComponent &> &second) {
  return first.first.GetPos().z < second.first.GetPos().z;
}

RenderSystemLegacy::RenderSystemLegacy(Ref<Scene> &scene) : GameSystem(scene) {
  glEnable(GL_BLEND);
...
void RenderSystemLegacy::Activate() {
  GameSystem::Activate();

  Application::Get().GetWindow()->OnWindowResize->AddListener(
      BIND_EVENT_HANDLER_2(RenderSystemLegacy::HandleOnWindowResize));
}

void RenderSystemLegacy::Update() {
  // stats
...
void RenderSystemLegacy::RenderSprites() {
  ...
  for (auto &&[entity, tr, render] :
       registry->view<TransformComponent, SpriteRendererComponent>().each()) {
    sprites.push_back(
        std::pair<TransformComponent &, SpriteRendererComponent &>(tr, render));
  }
  ...
  for (auto &it : sprites) {
    ...
    if (m_SpritesBuffer.m_SpritesCount >=
            m_SpritesBuffer.MAX_QUADS_COUNT_FOR_CALL ||
        m_SpritesBuffer.m_BindedTexturesCount >=
            m_SpritesBuffer.m_MaxTextureCount)
      FlushSprites();
  }

  if (m_SpritesBuffer.m_SpritesCount > 0)
    FlushSprites();
}

void RenderSystemLegacy::FlushSprites() {
  s_RenderStats.DrawCalls++;

  uint32_t dataSize = (uint32_t)((uint8_t *)m_SpritesBuffer.m_vertexDataPtr -
                                 (uint8_t *)m_SpritesBuffer.m_vertexDataHead);
  m_SpritesBuffer.m_vertexArray->GetVertexBuffer()->SetData(
      m_SpritesBuffer.m_vertexDataHead, dataSize);
  m_SpritesBuffer.m_vertexArray->Bind();
  m_SpritesBuffer.m_shader->Bind();
  glDrawElements(GL_TRIANGLES, m_SpritesBuffer.m_IndexCount, GL_UNSIGNED_INT,
                 nullptr);

  m_SpritesBuffer.Clear();
}

void RenderSystemLegacy::SubmitSpriteForRender(TransformComponent &tc,
                                               SpriteRendererComponent &sc,
                                               CameraComponent &camera) {
  if (m_SpritesBuffer.m_BindedTextures.find(sc.Texture) ==
      m_SpritesBuffer.m_BindedTextures.end()) {
  ...
  auto quadTr = camera.Camera->GetViewProjectionMatrix() * tc.ViewTransform();
  for (int i = 0; i < m_SpritesBuffer.QUAD_VERTEX_COUNT; i++) {
    m_SpritesBuffer.m_vertexDataPtr->Position =
        quadTr * m_SpritesBuffer.m_QuadVertexPositions[i];
    m_SpritesBuffer.m_vertexDataPtr->Color = sc.Color;
    m_SpritesBuffer.m_vertexDataPtr->TextureCoord = textureCoords[i];
    m_SpritesBuffer.m_vertexDataPtr->TextureSlot =
        m_SpritesBuffer.m_BindedTextures.find(sc.Texture)
            ->second->get()
            ->GetSlot();
...
bool RenderSystemLegacy::HandleOnWindowResize(uint32_t width, uint32_t height) {
  auto registry = m_scene->GetRegistry();
  auto camView = registry->view<CameraComponent>();
  CameraComponent &camera = camView.get<CameraComponent>(camView[0]);

  camera.AspectRatio = (float)width / (float)height;
  camera.Camera->SetProjectionMatrix(-camera.AspectRatio * camera.ZoomLevel,
