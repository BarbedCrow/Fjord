...
namespace Fjord
{
	...
	RenderSystemLegacy::RenderSystemLegacy(Ref<Scene>& scene)
		: GameSystem(scene)
	{
		glEnable(GL_BLEND);
	...
	void RenderSystemLegacy::RenderSprites()
	{
		...
		for (auto& it : sprites) {
			...
			if (m_SpritesBuffer.m_SpritesCount >= m_SpritesBuffer.MAX_QUADS_COUNT_FOR_CALL || m_SpritesBuffer.m_BindedTexturesCount >= m_SpritesBuffer.m_MaxTextureCount)
				FlushSprites();
		}
